...
## 0. MVP 기준 재정의 (2026-04-19)

이 앱의 MVP는 "길찾기 자체"보다 아래 4가지 운전 의도를 만족시키는 데 있다.

### MVP target 1
- 시간이 조금 더 늘어나더라도 정체를 피하고 싶어서, "다음 분기점으로 나가면 어디로 연결되고 얼마나 달라지는지"를 알고 싶은 사용자

### MVP target 2
- 시간이 조금 더 늘어나더라도 운전이 무섭고 복잡한 합류/차선 변경을 피하고 싶어서, 최대한 직진 위주와 단순한 분기 구조를 선호하는 초보 운전자

### MVP target 3
- ETA 최적화보다 드라이브 경험을 우선해, 바다와 산을 보며 달릴 수 있는 실제 경유 도로를 선택하고 싶은 사용자

### MVP target 4
- 특정 고속도로 또는 국도 자체를 달리는 것이 목적이라, 시점/종점 검색뿐 아니라 "현재 위치에서 그 도로를 가장 빨리 탈 수 있는 진입 루트"까지 원하는 사용자

### 비-MVP
- 주유소, 주차장, 병원, 맛집, 대중교통 링크, 카메라 제보, 즐겨찾기, 설정 유지, 시뮬레이터는 제품 완성도를 높이는 기능이지만 MVP의 본질은 아니다.
- 이 부가 기능들은 위 4가지 목표를 방해하지 않는 범위에서만 우선순위를 가진다.

### MVP 완료 조건
- 구현자가 이 문서만 보고도 아래 4개 사용 시나리오를 재현 가능한 기능으로 만들어야 한다.
  - 현재 경로를 유지할지, 다음 분기점으로 나가면 어디로 이어지는지 비교해서 선택
  - 초보 운전자가 복잡한 합류와 차선 변경을 줄인 경로를 선택
  - 해안도로/산악도로를 실제 waypoint로 넣어 경로에 반영
  - 특정 고속도로/국도를 선택하고 현재 위치에서 가장 빠른 진입 경로를 생성
...
### RR-123
- route preview 또는 안내 중 분기 비교 UI는 "다음 분기점으로 나가면 어디로 이어지는지"를 현재 경로와 나란히 보여줘야 하며, 최소 아래 값을 같이 표시해야 한다.
  - 현재 경로 유지 여부
  - 다음 분기점까지 남은 거리
  - 분기 후 이어질 도로명
  - 분기 후 이어질 도로 유형(고속도로/국도/일반도로)
  - 현재 경로 대비 추가 시간 또는 절약 시간
  - 난이도 또는 복잡도

### RR-124
- 초보 운전자용 경로 선택은 ETA 최적화만으로 결정하면 안 되며, 아래 항목을 합산한 "복잡도 우선" 판단을 별도 제공해야 한다.
  - 합류 횟수
  - 차선 변경 예상 횟수
  - 초반 5km 내 좌/우회전 횟수
  - 초반 5km 내 분기점 수
  - 일반도로 비중
  - 혼잡도가 높은 도심 구간 비중
- 초보 운전자용 추천의 기본 tie-break 순서는 아래와 같아야 한다.
  1. 합류 횟수가 더 적은 경로
  2. 초반 5km 내 분기점이 더 적은 경로
  3. 차선 변경 예상 횟수가 더 적은 경로
  4. 도심 혼잡 구간 비중이 더 낮은 경로
  5. 일반도로 비중이 더 낮은 경로
  6. 위 조건이 같을 때만 ETA가 더 짧은 경로

### RR-125
- 초보 운전자용 추천 경로는 가장 빠른 경로가 아니라도 선택될 수 있어야 하며, 최소 아래 설명 문구를 route card 또는 preset 설명에 포함해야 한다.
  - "직진 위주"
  - "초반 분기 적음"
  - "차로변경 여유"
  - "도심 혼잡 회피"
...
### RR-126
- scenic drive는 추천 카드만 보여주는 기능이 아니라 실제 경로 state에 반영되는 waypoint 기능이어야 하며, 선택 후 route summary에 아래 정보가 남아야 한다.
  - 어떤 scenic 구간이 추가됐는지
  - 어디서 진입하는지
  - 어디서 이탈하는지
  - 원래 경로 대비 얼마나 시간이 늘었는지

### RR-127
- 고속도로/국도 드라이브 모드는 사용자가 특정 도로를 선택하면, 현재 위치에서 해당 도로의 시점 또는 가장 가까운 유효 진입 지점까지 가는 연결 경로를 먼저 계산한 뒤 해당 도로 본선을 이어서 타는 route를 생성해야 한다.

### RR-128
- 고속도로/국도 드라이브 모드의 경로 생성은 단순히 시점/종점 좌표를 목적지로 넣는 방식이 아니라 아래 순서로 계산되어야 한다.
  - 현재 위치에서 진입 가능한 진입 후보(IC, JC, 시점, 앵커 노드)를 찾는다.
  - 각 후보까지의 연결 ETA를 계산한다.
  - 가장 빠르거나 사용자가 선택한 진입 후보를 기준으로 본선 route를 이어 붙인다.
  - 본선 진입 후 주요 JC/IC/휴게소 정보를 route context에 유지한다.
- 진입 후보가 여러 개일 때의 기본 처리 규칙은 아래와 같아야 한다.
  - 가장 빠른 후보와 두 번째 후보의 ETA 차이가 `2분 미만`이면 사용자 선택 UI를 먼저 보여준다.
  - ETA 차이가 `2분 이상`이면 가장 빠른 후보를 기본 선택한다.
  - 사용자가 수동으로 특정 진입 후보를 골랐으면 ETA가 더 느려도 그 후보를 유지한다.

### RR-129
- 도로를 "타는 것 자체"가 목적인 사용자를 위해, 검색 결과 또는 도로 탐색 UI에는 일반 POI와 다른 "도로 진입" 액션이 있어야 하며 최소 아래 2개 액션 중 하나를 제공해야 한다.
  - `이 도로 타러 가기`
  - `가장 빠른 진입로 안내`

### RR-130
- 주유소, 주차장, 병원, 맛집, 외부 교통 링크는 route preview의 기본 노출 순서를 밀어내면 안 되며, MVP 핵심 UI보다 뒤에 배치되어야 한다.
...
- 원본 요구사항 번호 `RR-001 ~ RR-130`은 유지한다.
...
### FG-05. 다음 분기점 이해와 분기 결과 비교

- 우선순위: `P0`
- 목적: 사용자가 "지금 나가면 어디로 이어지는지"를 ETA가 아니라 도로 연결 관점에서도 이해할 수 있게 만든다.
- 포함 요구:
  - `RR-085`, `RR-086`
  - `RR-088`, `RR-089`, `RR-090`, `RR-091`
  - `RR-123`
- 판정 기준:
  - 다음 분기점까지 거리와, 분기 후 이어질 도로명이 명시된다.
  - 현재 경로 유지와 분기 후 대체 연결이 한 화면에서 비교된다.

### FG-06. 초보 운전자용 직진/단순 경로

- 우선순위: `P0`
- 목적: 가장 빠른 경로가 아니라도, 초보 운전자가 덜 무섭고 덜 복잡한 경로를 선택할 수 있게 만든다.
- 포함 요구:
  - `RR-046`, `RR-047`
  - `RR-124`, `RR-125`
- 판정 기준:
  - 추천 결과에 ETA 외에 복잡도 기준이 분리된다.
  - 초보 추천은 합류/차선변경/초반 분기 수가 적은 경로를 우선한다.

### FG-07. 좋은 도로 추천의 실제 waypoint화
...
- 판정 기준:
  ...
  - 진입점과 이탈점이 명시되고 역주행 필요 시 경고가 나온다.

### FG-08. 특정 고속도로/국도 자체를 타는 드라이브 모드

- 우선순위: `P0`
- 목적: 특정 도로를 달리는 것이 목적일 때, 현재 위치에서 해당 도로를 가장 빨리 탈 수 있는 진입 루트를 만든다.
- 포함 요구:
  - `RR-100`, `RR-101`, `RR-102`, `RR-104`
  - `RR-127`, `RR-128`, `RR-129`
- 판정 기준:
  - 도로를 고르면 해당 도로의 진입 후보를 계산한다.
  - 현재 위치에서 가장 빠른 진입 경로를 만든 뒤 본선 route가 이어진다.
  - UI에 `이 도로 타러 가기` 또는 `가장 빠른 진입로 안내` 액션이 있다.

### FG-09. 다음 안내 음성과 주행 중 최소 안내
...
- 판정 기준:
  ...
  - 운전 중 사용자가 "지금 다음에 뭘 해야 하는지"를 소리와 배너 둘 다로 이해할 수 있다.

### FG-10. Dev/Prod 검증과 회귀 확인
...
## P1 — MVP를 더 정확하고 더 읽기 좋게 만드는 요구

### FG-11. 운전자 시점 지도와 시야 정리
...
### FG-13. 도로 정보 패널과 내부 요약

- 우선순위: `P1`
- 목적: 선택한 도로의 구조와 가까운 도로 흐름을 내부 데이터로 요약해 보여준다.
- 포함 요구:
  - `RR-103`
- 판정 기준:
  - 선택한 도로 또는 가까운 도로의 요약 정보가 외부 링크 없이 먼저 보인다.

## P2 — 부가 안내, 운영 효율, 확장 경험

### FG-14. 맛집/주유소/병원/주차장 실사용 정보
...
- 판정 기준:
  ...
  - 병원/주차장은 가능한 메타가 붙는다.
  - 이 기능들이 route preview의 핵심 UI보다 앞에 나오지 않는다.
...
### FG-20. Fast candidate와 탐색 체감 속도 최적화
...
4. `FG-05` 다음 분기점 이해와 분기 결과 비교
5. `FG-06` 초보 운전자용 직진/단순 경로
6. `FG-08` 특정 고속도로/국도 자체를 타는 드라이브 모드
7. `FG-07` 좋은 도로 추천의 실제 waypoint화
8. `FG-09` 다음 안내 음성과 주행 중 최소 안내
9. `FG-04` 검색, 출발지, 목적지, 경유지의 핵심 플로우
10. `FG-12` 차선, 분기, 합류 판단 고도화
