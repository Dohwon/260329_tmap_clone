...
- Google TTS를 매번 다시 합성하지 않도록 `mp3 파일 캐시 + 메타 인덱스` 구조를 고정했다.
  - `server.js`
  - 저장 위치는 `/data` 우선, 없으면 `.runtime-cache`
  - `tts-google/*.mp3`로 실제 음성 파일을 저장하고 `tts-google-index.json`에 메타를 남긴다.
  - 메타에는 원문 전체가 아니라 `voiceName`, `languageCode`, `speakingRate`, `textLength`, `createdAt`, `lastAccessAt`, `hitCount`, `sizeBytes`만 저장한다.
  - 같은 문구/목소리/속도 조합은 SHA1 cache key 기준으로 재사용한다.
  - 오래된 음성은 `180일 TTL`과 `최대 5000개` 기준으로 정리한다.

### 재확인된 병목

- “캐시가 있다”와 “운영 중 실제로 재사용된다”는 다르다.
  - 단순 mp3 파일만 저장하면 어떤 항목이 살아 있고 얼마나 재사용됐는지 보이지 않는다.
  - 재배포 후 볼륨이 붙어 있어도 메타가 없으면 정리 기준과 운영 판단이 어렵다.

### 해결 방식

- TTS 캐시를 브라우저 캐시가 아니라 서버 볼륨 캐시로 고정했다.
- 재사용 여부는 응답 헤더 `X-TTS-Cache: HIT/MISS`와 `X-TTS-Cache-Store`로 바로 확인 가능하게 했다.
- 파일 캐시와 인덱스를 같이 두어, “매번 합성하지 말라”는 요구를 운영 가능한 형태로 정리했다.

### 다음 액션

1. 실제 배포 환경에서 `/data` 볼륨 유지 여부를 계속 본다.
2. 자주 쓰는 기본 안내 멘트는 prewarm 대상으로 묶을지 검토한다.
